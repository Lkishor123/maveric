...
class BDTManager:
    """
    Manages training of the Bayesian Digital Twin model and downloads the result.
    """
    ...
    def train(self, model_id: str, container_name: str):
        ...
        try:
            ...
            radp_client.train(
                model_id=model_id, params={},
                ue_training_data=training_data_df, topology=topology_df, model_update=False
            )
            logger.info(f"Training request sent for model_id: {model_id}. Waiting for completion...")
            status: ModelStatus = radp_helper.resolve_model_status(model_id, wait_interval=30, max_attempts=120, verbose=True)
