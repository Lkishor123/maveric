# main_app.py
# Main orchestrator for the Energy Saving RL application pipeline.
...
# Assumes this app sits in a directory like 'apps/energy_saving_app/',
# so two levels up would be the project root.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Energy Saving Application using BDT and RL.",
        formatter_class=argparse.RawTextHelpFormatter
    )
    parser.add_argument("--preprocess-data", action="store_true", help="Step 1: Preprocess raw UE data CSVs for Gym compatibility.")
    parser.add_argument("--train-bdt", action="store_true", help="Step 2: Train the Bayesian Digital Twin model map via the backend.")
    ...
    parser.add_argument("--visualize", action="store_true", help="Step 5: Generate comparison plots for a given tick on a test day.")
    ...
    parser.add_argument("--total-timesteps", type=int, default=48000, help="Total timesteps for RL training (for --train-rl).")
    parser.add_argument("--bdt-model-id", type=str, default="bdt_energy_saving_v1", help="Unique ID for the BDT model being trained on the backend.")
    ...
    if args.infer:
        if args.tick is None: parser.error("--tick is required for inference.")
    ...
    if args.visualize:
        if args.tick is None: parser.error("--tick is required for visualization.")
