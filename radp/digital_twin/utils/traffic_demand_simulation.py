...
class TrafficDemandModel:
    ...
    def _assign_space_types_to_polygons(
        self, polygons: List[Polygon], space_type_proportions: Dict[str, float]
    ) -> List[Dict[str, Any]]:
        """
            ...
            spatial_cells_with_types.append(
                {
                    ...
                    "type": assigned_type,
                    "spatial_cell_id": i,  # Simple ID for this spatial cell
                    "area_sq_km": GISTools.area_of_polygon(poly.exterior.coords)
                    if hasattr(GISTools, "area_of_polygon")
                    else poly.area,  # poly.area is in squared units of coords
                }
            )

        return spatial_cells_with_types

    def _generate_spatial_layout(
        self, topology_df: pd.DataFrame, spatial_params: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if len(points) < 4:  # Voronoi requires at least N+1 points in N-D
            logger.error(
                f"Need at least 4 cell sites for 2D Voronoi, found {len(points)}. Cannot generate spatial layout."
            )
            return []
                    ...
                    for poly_geom in region_polygon.geoms:
                        if poly_geom.geom_type == "Polygon":
                            ...
                            if (
                                ...
                                and clipped_poly.is_valid
                                and clipped_poly.geom_type == "Polygon"
                                and clipped_poly.area > 1e-9
                            ):  # Area threshold
        ...
        logger.info(
            f"""Voronoi processing:
            Regions processed={processed_regions},
            Infinite skipped={skipped_infinite},
            Invalid/Empty skipped={skipped_invalid_geom}
            """
        )
    ...
    def _distribute_ues_in_layout(
        self, spatial_layout: List[Dict[str, Any]], time_params: Dict[str, Any], num_ues_per_tick: int
        ...
        for tick in range(total_ticks):
            ...
            for space_type, num_ues_for_type in target_counts_for_tick.items():
                ...
                if not available_cells_for_this_type:
                    logger.warning(
                        f"""Cannot place {num_ues_for_type} UEs for type '{space_type}' at tick {tick}:
                        No spatial cells of this type in layout."""
                    )
                    continue
                ...
                for _ in range(num_ues_for_type):
                    ...
                    while attempts < max_attempts:
                            ...
                            tick_ue_data_list.append(
                                {
                                    ...
                                    "voronoi_cell_id": chosen_cell_info["spatial_cell_id"],
                                }
                            )
                        ...
                        logger.warning(
                            f"""Failed to place UE in cell {chosen_cell_info['spatial_cell_id']}
                            (type '{space_type}') after {max_attempts} attempts for tick {tick}."""
                        )
...
if __name__ == "__main__":
    ...
    example_spatial_params = {"types": ["residential", "commercial", "park"], "proportions": [0.5, 0.3, 0.2]}
    example_time_params = {
        "total_ticks": 3,  # Short simulation for example
        "time_weights": {"residential": [0.8, 0.7, 0.6], "commercial": [0.1, 0.2, 0.3], "park": [0.1, 0.1, 0.1]},
    }
    example_num_ues = 100
