...
class TrafficDemandModel:
    ...
    def _assign_space_types_to_polygons(self, polygons: List[Polygon], spatial_params: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Deterministically assigns space types to a list of polygons based on given proportions.
        """
    ...
    def generate_spatial_layout(self,
        ...
        """
        Generates a fixed spatial layout using Voronoi tessellation. This method is
        robust against few sites by adding temporary bounding points to ensure all
        real sites produce a finite polygon.
        """
        ...
        # --- NEW: Add temporary "ghost" points to bound the diagram ---
        # This ensures all regions for the original points are finite.
        ...
        # MODIFIED: Loop through original points and get their (now finite) regions
        valid_polygons = []
    ...
    def distribute_ues_over_time(self,
        ...
        """
        Distributes UEs across a pre-generated spatial layout for each time tick.
        """
        ...
        for tick in range(total_ticks):
            ...
            # Use a temporary dict to handle rounding and ensure total UEs match the target
            ue_counts = {}
            for space_type, weight in tick_type_weights.items():
                ue_counts[space_type] = (weight / total_weight_this_tick) * num_ues_per_tick
            
            # Adjust counts to integers, ensuring the sum matches num_ues_per_tick
            total_ues = 0
